...
@ManagedBean
@ViewScoped
public class AclGroupActionBC
  implements Serializable
{
  private String userId = AccessControl.getUserid(ContextUtils.getRequest());
  private String regGroupId = ContextUtils.getRequest("groupId", "0");
  ...
  public void bind()
  {
    if (Integer.valueOf(this.regGroupId).intValue() > 0)
    {
      Map<String, String> group = AclGroupDAO.getById(this.regGroupId);
      if (group == null)
      {
        ContextUtils.addMessage(MessageFormat.format(LanguageBC.getValue("message.objectNotExist"), new Object[] { LanguageBC.getValue("label.group") }));
      }
      else
      {
        ...
        List<Map<String, String>> lstAllowActionRaw = AclActionObjectDAO.getListAllow("1", this.objectType, this.regGroupId);
        ...
        List<Map<String, String>> lstRestrictActionRaw = AclActionObjectDAO.getListAllow("0", this.objectType, this.regGroupId);
  ...
  public void allow()
  {
    if (!StringUtils.isBlank(this.lstFreeActionId))
    {
      ...
      for (String actionId : actionIds)
      {
        ...
        map.put("object_id", this.regGroupId);
  ...
  public void removeAllow()
  {
    if (!StringUtils.isBlank(this.lstAllowActionId))
    {
      ...
      for (String actionId : actionIds) {
        if (!AclActionObjectDAO.delete(actionId, this.regGroupId, this.objectType))
        {
          succesQuery = false;
          break;
        }
